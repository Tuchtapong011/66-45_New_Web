...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ตรวจสอบว่ำกรอกข ้อมูลมำครบหรือไม่ (empty)
    if(empty($username)||empty($fullname)||empty($email)||empty($password)||empty($Confirm_Password)){
        ...
        // ตรวจสอบว่ารหัสผ่านและยีนยันรหัสผ่านตรงกันหรือไม่
        $error[] = "รหัสผ่านและยีนยันรหัสผ่านไม่ตรงกัน";
    } else {
        ...
        if($stmt->rowCount() > 0){
            $error[] = "ชื่อผู้ใข้หรืออีเมลนี้ถูกใข้ไปแล้ว";
        }
    }
    ...
    if (empty($error)) { // ถ้าไม่มีข้อผิดพลาดใดๆ
        ...
        if ($password !== $Confirm_Password) {
            ...
            // ถ้า save สำเร็จ ให้เปลี่ยนเส้นทางไปหน้า login
            header("Location: login.php?register=success");
            exit(); // หยุดรันเมื่อโค้ดทำงานผิดพลาด
        }
    ...
    <div class="container">
        ...
        <form action="" method="post">
            ...
            <div class="mb-3">
                ...
                <input type="password" name="Confirm_Password" id="ConfirmPassword" class="form-control"placeholder="ยืนยันรหัสผ่าน" required>
            </div>
            <div class="mt-3">
